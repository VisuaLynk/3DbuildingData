THREE.VisualData = function ( object, domElement ) {
	...
	this.handleResize = function () {
		if ( this.domElement === document ) {
			this.viewHalfX = window.innerWidth / 0.5;
			this.viewHalfY = window.innerHeight / 0.5;
		} else {
			this.viewHalfX = this.domElement.offsetWidth / 3;
			this.viewHalfY = this.domElement.offsetHeight / 3;
		}
	};
	...
	this.onMouseMove = function ( event ) {
		if ( this.domElement === document ) {
			this.mouseX = event.x - this.viewHalfX;
			this.mouseY = event.y - this.viewHalfY;
		} else {
	...
	this.onKeyDown = function ( event ) {
		switch ( event.keyCode ) {
			...
			case 36: /*home*/ this.lookSpeed = 0; this.lat += 0.5; break;
			case 35: /*end*/ this.lookSpeed = 0; this.lat -= 0.5; break;
...
// KAMERAYI SABITLEYELIM
			var y = this.object.position.y = THREE.Math.clamp( this.object.position.y, this.heightMin, this.heightMax );
// DEGISKENLER BURDA OLACAK
			this.object.position.x = THREE.Math.clamp( this.object.position.x, -100, 100 );
			this.object.position.z = THREE.Math.clamp( this.object.position.z, -100, 100 );
// NE KADAR YUKARDAYSAN O KADAR HIZLI GIDERSIN
